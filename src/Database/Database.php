...
class Database {
    ...
    protected function addFieldValuePairs(array $pairs) {
        foreach($pairs as $field => $value) {
            array_push($this->fieldValuePairs, "`{$field}` = " . (is_string($value) && !in_array($value, array('NOW()', 'CURDATE()')) ? "'{$this->escape($value)} '" : $value));
        }
    }
    ...
    protected function addValues(array $values) {
        foreach($values as $value) {
            array_push($this->values, (is_string($value) && !in_array($value, array('NOW()', 'CURDATE()')) ? "'{$this->escape($value)}'" : $value));
        }
    }
    ...
    public function get() {
        if(func_num_args() === 1 && is_array(func_get_arg(0))) {
            ...
            throw new BadFunctionCallException('Invailid parameters for `Zoom\Database@get()` method');
        }
    ...
    public function orWhere() {
        if( func_num_args() === 3) {
            ...
            throw new BadFunctionCallException('Database\Zoom@orWhere expects at least two parameters! Less given.');
        }
        return $this;
    }
    ...
    public function where() {
        if(func_num_args() === 3) {
            ...
            if (is_array(func_get_arg(0))) {
                ...
                throw new BadFunctionCallException('\Zoom\Database@where expects the parameter given to be an array!');
            }
        } else {
            throw new BadFunctionCallException('\Zoom\Database@where expects at least two parameters! Less given.');
        }
        return $this;
    }
}
